...
var ASM_CONSTS = {
  4557396: () => { Module['emscripten_get_now_backup'] = performance.now; },  
 4557451: ($0) => { performance.now = function() { return $0; }; },  
 4557499: ($0) => { performance.now = function() { return $0; }; },  
 4557547: () => { performance.now = Module['emscripten_get_now_backup']; }
};
...
// end include: preamble.js
  ...
  function WS_DynCall_viii(funcPtr, a, b, c) {
  		if (typeof dynCall_viii === "function") return dynCall_viii(funcPtr, a, b, c);
  		return WS_ResolveFunc(funcPtr)(a, b, c);
  	}
  function WS_DynCall_vii(funcPtr, a, b) {
  		if (typeof dynCall_vii === "function") return dynCall_vii(funcPtr, a, b);
  		return WS_ResolveFunc(funcPtr)(a, b);
  	}
  
  function WS_DynCall_vi(funcPtr, a) {
  		if (typeof dynCall_vi === "function") return dynCall_vi(funcPtr, a);
  		return WS_ResolveFunc(funcPtr)(a);
  	}
  
  
  
  function WS_ResolveFunc(funcPtr) {
  		if (typeof getWasmTableEntry === "function") return getWasmTableEntry(funcPtr);
  		if (typeof wasmTable !== "undefined" && wasmTable) return wasmTable.get(funcPtr);
  		if (typeof Module !== "undefined" && Module.wasmTable) return Module.wasmTable.get(funcPtr);
  		throw new Error("WebSocket.jslib: Unable to resolve wasm function pointer " + funcPtr);
  	}
  ...
  function _WebSocketConnect(instanceId) {
  		...
  		instance.ws.onopen = function() {
  			...
  			if (webSocketState.onOpen)
  				WS_DynCall_vi(webSocketState.onOpen, instanceId);
  		...
  		instance.ws.onmessage = function(ev) {
  			...
  			if (ev.data instanceof ArrayBuffer) {
  				...
  				try {
  					WS_DynCall_viii(webSocketState.onMessage, instanceId, buffer, dataBuffer.length);
  				} finally {
  					_free(buffer);
  				}
  		...
  		instance.ws.onerror = function(ev) {
  			...
  			if (webSocketState.onError) {
  				...
  				try {
  					WS_DynCall_vii(webSocketState.onError, instanceId, buffer);
  				} finally {
  					_free(buffer);
  				}
  		...
  		instance.ws.onclose = function(ev) {
  			...
  			if (webSocketState.onClose)
  				WS_DynCall_vii(webSocketState.onClose, instanceId, ev.code);
...
var wasmImports = {
  "DS_IsMobileBrowser": _DS_IsMobileBrowser,
